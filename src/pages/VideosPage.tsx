...
export function VideosPage() {
  ...
  const [isBootstrappingSession, setIsBootstrappingSession] = useState(Boolean(token))
  const [isSubmittingAuth, setIsSubmittingAuth] = useState(false)
  const [isRefreshingVideos, setIsRefreshingVideos] = useState(false)
  const [isUploading, setIsUploading] = useState(false)
  const [isDragActive, setIsDragActive] = useState(false)
  const [deletingVideoId, setDeletingVideoId] = useState<string | null>(null)
  const [videoPendingDelete, setVideoPendingDelete] = useState<ApiVideo | null>(null)
  ...
  useEffect(() => {
    if (selectedWeek && !availableWeeks.some((week) => week.key === selectedWeek)) {
      setSelectedWeek('')
    }
  ...
  useEffect(() => {
    ...
    async function bootstrapPrivateArea() {
      ...
      try {
        ...
        const message = error instanceof Error ? error.message : 'No se pudo cargar la sesion del profesor'
        setToken('')
        setCurrentUser(null)
        setNotice({
          tone: 'error',
          message: message.includes('Token') ? 'La sesion vencio. Volve a iniciar sesion.' : message,
        })
      } finally {
        if (!ignore) {
          setIsBootstrappingSession(false)
  ...
  async function refreshVideos() {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : 'No se pudo actualizar la biblioteca'
      setNotice({ tone: 'error', message })
    } finally {
  ...
  function handleFilesAdded(files: File[]) {
    ...
    setUploadQueue((current) => [...current, ...acceptedFiles.map(createUploadDraft)])
  ...
  function updateDraft(id: string, updater: (draft: UploadDraft) => UploadDraft) {
    setUploadQueue((current) => current.map((draft) => (draft.id === id ? updater(draft) : draft)))
  }

  function handleDraftTextChange(
    id: string,
    field: 'title' | 'description' | 'category' | 'publishedAt' | 'durationLabel',
    value: string,
  ) {
  ...
  async function handleUploadQueue() {
    ...
    for (const draft of uploadQueue) {
      ...
      try {
        ...
        failedCount += 1
        const message = error instanceof Error ? error.message : 'No se pudo subir el video'
    ...
    setUploadQueue((current) => current.filter((draft) => draft.status === 'error'))
  ...
  return (
    <main className="px-6 pb-24 pt-12 lg:px-10 lg:pb-28 lg:pt-16">
      <section className="mx-auto max-w-7xl">
          ...
          >
            ...
            >
              ...
              <div className="relative">
                <p className="text-xs uppercase tracking-[0.3em] text-[var(--color-accent)]">Confirmar borrado</p>
                <h2 className="mt-4 font-display text-4xl uppercase tracking-[0.08em] text-white">
                  Sacar este video de la biblioteca.
                </h2>
                <p className="mt-4 text-sm leading-7 text-white/68">
                  Vas a quitar <span className="text-white">{videoPendingDelete.title}</span> de la biblioteca publica.
                  Los videos viejos pueden seguir existiendo en YouTube aunque desaparezcan de esta pagina.
                </p>

                <div className="mt-6 rounded-[1.35rem] border border-white/10 bg-black/18 p-4">
                  <p className="text-[0.68rem] uppercase tracking-[0.24em] text-white/45">Video seleccionado</p>
                  ...
                  <p className="mt-2 text-xs uppercase tracking-[0.22em] text-white/52">
                    {videoPendingDelete.category || 'Sin categoria'} · {formatVideoDate(videoPendingDelete.publishedAt)}
                  </p>
                </div>
                ...
                <div className="mt-8 flex flex-wrap justify-end gap-3">
                  ...
                  >
                    {deletingVideoId === videoPendingDelete.id ? 'Borrando...' : 'Borrar video'}
                  </button>
                </div>
              </div>
            </div>
          ...
          <div className="relative grid gap-8 xl:grid-cols-[1.15fr_0.85fr] xl:items-start">
            <div>
              ...
              <p className="mt-5 max-w-2xl text-base leading-8 text-white/72 md:text-lg">
                Cualquier persona puede entrar y ver los videos del club. Si inicias sesion como profesor, esta pantalla permite revisar la biblioteca y borrar material publicado.
              </p>
              <p className="mt-4 max-w-2xl text-sm leading-7 text-white/58">
                Desde esta pantalla el profesor puede seleccionar o arrastrar archivos, revisarlos antes de enviarlos y publicarlos en YouTube.
              </p>

              <div className="mt-8 grid gap-4 md:grid-cols-3">
                <div className="rounded-[1.4rem] border border-white/10 bg-black/18 p-4">
                  <p className="text-[0.68rem] uppercase tracking-[0.28em] text-white/48">Acceso</p>
                  <p className="mt-3 font-display text-3xl uppercase tracking-[0.08em] text-white">Publico</p>
                  <p className="mt-2 text-sm leading-6 text-white/62">La biblioteca se puede recorrer sin iniciar sesion.</p>
                </div>
                <div className="rounded-[1.4rem] border border-white/10 bg-black/18 p-4">
                  <p className="text-[0.68rem] uppercase tracking-[0.28em] text-white/48">Profesor</p>
                  <p className="mt-3 font-display text-3xl uppercase tracking-[0.08em] text-white">Borra</p>
                  <p className="mt-2 text-sm leading-6 text-white/62">Solo el profesor puede iniciar sesion para retirar material de la biblioteca.</p>
                </div>
                <div className="rounded-[1.4rem] border border-white/10 bg-black/18 p-4">
                  <p className="text-[0.68rem] uppercase tracking-[0.28em] text-white/48">Reproduccion</p>
                  <p className="mt-3 font-display text-3xl uppercase tracking-[0.08em] text-white">YouTube</p>
                  <p className="mt-2 text-sm leading-6 text-white/62">El material visible se reproduce embebido desde YouTube.</p>
                </div>
              </div>
            </div>
              ...
              {isAuthenticated ? (
                <div>
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <p className="text-xs uppercase tracking-[0.28em] text-[var(--color-accent)]">Profesor conectado</p>
                  ...
                  <div className="mt-6 rounded-[1.3rem] border border-white/10 bg-white/5 p-4">
                    <p className="text-[0.68rem] uppercase tracking-[0.26em] text-white/42">Permisos</p>
                    <p className="mt-3 text-sm leading-7 text-white/70">
                      Tu cuenta puede subir y borrar videos. La cabina de carga aparece debajo.
                    </p>
                  </div>
                  ...
                  <div className="mt-6 flex flex-wrap gap-3">
                    <button
                      ...
                      className={secondaryButtonClassName}
                    >
                      {isRefreshingVideos ? 'Actualizando...' : 'Actualizar biblioteca'}
                    </button>
                    <button
                      type="button"
                ...
                <div>
                  <div className="mt-6">
                    <p className="text-xs uppercase tracking-[0.28em] text-[var(--color-accent)]">Panel del profesor</p>
                    <h2 className="mt-3 font-display text-4xl uppercase tracking-[0.08em] text-white">
                      Iniciar sesion
                    </h2>
                    <p className="mt-3 text-sm leading-7 text-white/62">
                      La biblioteca es publica. El login queda reservado al profesor para borrar material.
                    </p>
                  </div>

                  <form className="mt-6 flex flex-col gap-4" onSubmit={handleAuthSubmit}>
                    ...
                    <button type="submit" disabled={isSubmittingAuth} className={primaryButtonClassName}>
        ...
        {notice ? (
          <div className={`mt-6 rounded-[1.35rem] border px-5 py-4 text-sm leading-7 ${getNoticeStyles(notice.tone)}`}>
            {notice.message}
          </div>
        ) : null}
        ...
        {isProfessor ? (
          <section className="mt-8 rounded-[1.9rem] border border-white/10 bg-[linear-gradient(145deg,rgba(255,255,255,0.08),rgba(4,10,20,0.9)_60%)] p-6 md:p-8">
            <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
              <div>
                <p className="text-xs uppercase tracking-[0.3em] text-[var(--color-accent)]">Cabina del profesor</p>
                <h2 className="mt-3 font-display text-5xl uppercase tracking-[0.08em] text-white">
                  Arrastra videos y publicalos.
                </h2>
                <p className="mt-3 max-w-3xl text-sm leading-7 text-white/65">
                  Puedes arrastrar uno o varios archivos. Cada uno entra a una cola editable antes de subirse a YouTube y guardarse con su metadata en la biblioteca.
                </p>
              </div>
            ...
            >
              <p className="text-xs uppercase tracking-[0.3em] text-white/45">Zona de carga</p>
              <h3 className="mt-3 font-display text-4xl uppercase tracking-[0.08em] text-white">
                {isDragActive ? 'Solta los archivos aca.' : 'Tira tus videos sobre esta cabina.'}
              </h3>
              <p className="mt-3 max-w-2xl text-sm leading-7 text-white/62">
                Acepta MP4, MOV, WEBM y MKV. Cada item de la cola puede corregirse antes de subirlo, incluyendo tipo, fecha, categoria y descripcion.
              </p>
            </div>
            ...
            {uploadQueue.length ? (
                ...
                {uploadQueue.map((draft) => (
                  ...
                  >
                    <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                      <div>
                        <p className="text-xs uppercase tracking-[0.25em] text-white/44">Archivo listo</p>
                        <h3 className="mt-2 font-display text-3xl uppercase tracking-[0.08em] text-white">
                          {draft.file.name}
                        </h3>
                        <p className="mt-2 text-sm text-white/58">{formatBytes(draft.file.size)}</p>
                      </div>

                      <div className="flex items-center gap-3">
                        <span className={`rounded-full border px-4 py-2 text-[0.68rem] font-semibold uppercase tracking-[0.22em] ${getUploadStatusStyles(draft.status)}`}>
                    ...
                    <div className="mt-5 grid gap-4 xl:grid-cols-2">
                      <label className="flex flex-col gap-2 text-xs uppercase tracking-[0.22em] text-white/55">
                        ...
                        <input
                          type="text"
                          value={draft.title}
                          onChange={(event) => handleDraftTextChange(draft.id, 'title', event.target.value)}
                          className={inputClassName}
                          maxLength={180}
                          required
                        />
                      ...
                      <label className="flex flex-col gap-2 text-xs uppercase tracking-[0.22em] text-white/55">
                        ...
                        <input
                          ...
                          onChange={(event) => handleDraftTextChange(draft.id, 'category', event.target.value)}
                          className={inputClassName}
                      ...
                      <label className="flex flex-col gap-2 text-xs uppercase tracking-[0.22em] text-white/55">
                        ...
                        <select
                          ...
                          onChange={(event) => handleDraftTypeChange(draft.id, event.target.value as TrainingType)}
                          className={inputClassName}
                        >
                      ...
                      <label className="flex flex-col gap-2 text-xs uppercase tracking-[0.22em] text-white/55">
                        ...
                        <input
                          ...
                          onChange={(event) => handleDraftTextChange(draft.id, 'publishedAt', event.target.value)}
                          className={inputClassName}
                          required
                        />
                      </label>
                      ...
                      <label className="flex flex-col gap-2 text-xs uppercase tracking-[0.22em] text-white/55 xl:col-span-2">
                        ...
                        <input
                          ...
                          onChange={(event) => handleDraftTextChange(draft.id, 'durationLabel', event.target.value)}
                          className={inputClassName}
                      ...
                      <label className="flex flex-col gap-2 text-xs uppercase tracking-[0.22em] text-white/55 xl:col-span-2">
                        ...
                        <textarea
                          ...
                          onChange={(event) => handleDraftTextChange(draft.id, 'description', event.target.value)}
              ...
              <div className="mt-6 rounded-[1.6rem] border border-white/10 bg-black/20 p-6 text-sm leading-7 text-white/60">
                La cola esta vacia. Arrastra archivos o usa el boton de seleccion para preparar la proxima tanda.
              </div>
            )}
          </section>
        ) : (
          <section className="mt-8 rounded-[1.75rem] border border-dashed border-white/14 bg-black/18 p-6 md:p-8">
            <p className="text-xs uppercase tracking-[0.3em] text-[var(--color-accent)]">Acceso publico</p>
            <h2 className="mt-3 font-display text-4xl uppercase tracking-[0.08em] text-white">
              Entra sin login y mira la biblioteca.
            </h2>
            <p className="mt-4 max-w-3xl text-sm leading-7 text-white/64">
              Los videos publicados se cargan para cualquier visitante. Si eres el profesor, inicia sesion arriba para borrar material.
            </p>
          </section>
        )}
            ...
            <div className="mt-8 rounded-[1.75rem] border border-white/10 bg-white/5 p-6">
              <div className="flex flex-col gap-8">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                  <div>
                    <p className="text-xs uppercase tracking-[0.28em] text-white/50">Biblioteca publica</p>
                  ...
                  <p className="text-sm uppercase tracking-[0.22em] text-white/55">
                    {visibleVideos.length} video(s) {selectedWeek || selectedType !== 'todos' ? 'filtrados' : 'disponibles'}
                  </p>
                </div>

                <div className="grid gap-6 xl:grid-cols-[1.45fr_0.85fr]">
                  <div>
                    <p className="text-xs uppercase tracking-[0.22em] text-white/60">Semana</p>
                  ...
                  <div>
                    <p className="text-xs uppercase tracking-[0.22em] text-white/60">Tipo</p>
                    <div className="mt-3 flex flex-wrap gap-3">
                      {(['todos', 'entrenamiento', 'guanteo', 'exhibicion'] as const).map((type) => (
                        <button
                          key={type}
                          type="button"
                ...
                {visibleWeekGroups.map((weekGroup) => (
                  ...
                  >
                    <div className="flex flex-col gap-3 border-b border-white/10 pb-5 md:flex-row md:items-end md:justify-between">
                      <div>
                        <p className="text-xs uppercase tracking-[0.28em] text-[var(--color-accent)]">Semana</p>
                    ...
                    <div className="mt-6 flex flex-col gap-10">
                      {(['entrenamiento', 'guanteo', 'exhibicion'] as const).map((type) => {
                              ...
                              {videosByType.map((video) => (
                                ...
                                >
                                  {isYoutubeVideo(video) ? (
                                    <iframe
                                      src={resolveMediaUrl(video.embedUrl || '')}
                                      title={video.title}
                                      loading="lazy"
                                  ...
                                  <div className="p-6">
                                    <div className="flex flex-wrap gap-3 text-xs uppercase tracking-[0.22em] text-white/55">
                                      <span>{video.category || 'Sin categoria'}</span>
                                    ...
                                    <p className="mt-4 text-sm leading-7 text-white/70">
                                      {video.description || 'Video publicado en la biblioteca del club.'}
                                    </p>

                                    <div className="mt-6 flex flex-wrap items-center justify-between gap-4 border-t border-white/10 pt-5">
                                      <div className="text-xs uppercase tracking-[0.22em] text-white/48">
                                        <p>Subido por {video.uploadedBy.name}</p>
                                      </div>
                                      ...
                                      <div className="flex flex-wrap gap-3">
                                        ...
                                        {isProfessor ? (
                                          <button
                                            ...
                                            className={primaryButtonClassName}
                                          >
                                            {deletingVideoId === video.id ? 'Borrando...' : 'Borrar'}
                                          </button>
                                        ) : null}
                                      </div>
                                    </div>
